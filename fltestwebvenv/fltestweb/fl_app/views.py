...
def check(request,id):
    print("it is in check")
    try:
        os.remove("jasper0.png")
        os.remove("jasper1.png")
    except:
        print("not removed")
        pass
    data1 = models.Fl_test.objects.filter(ID = id).all()
    data2 = models.Voltage_data.objects.filter(ID = id).all()

    # data3 = models.Laser_data.objects.filter(ID = 1).only("TOP_VALUE")

    values = models.Laser_data.objects.filter(ID = id)
    ids = ""
    mac =""
    top = ""
    btm = ""
    ...
    plt.xlabel('TIME')
    plt.ylabel('BOTTOM WAVE voltage')
    plt.legend()
...
def test(request):
    try:
        nm = nmap.PortScanner()
        # nm.scan(hosts='192.168.30.0/24', arguments='-sn')
        print("nmap start scanning....")
        nm.scan(hosts='192.168.30.100-254', arguments='-sn')
    except:
        print("something wrong with nmap scanner")
    global on_flag
    if con_flag == True:
        command = "START_TEST"
        try:    
